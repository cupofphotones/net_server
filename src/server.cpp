...
public:
    ...
    void handlePart(const MessageHeader& header, std::istream& stream)
    {
       ...
       if (header.has("Content-Disposition"))
        ...
        StreamCopier::copyStreamUnbuffered(stream, MyFile);
...
public:
    ...
    void handleRequest(HTTPServerRequest& request, HTTPServerResponse& response)
    {
        ...
        std::string tpe = "image";

        if (request.getMethod() != "POST") {
            ostr << "Please provide POST request\n";
        }
        else {

            if (!partHandler.fileName().empty() && partHandler.contentType().find(tpe) != std::string::npos) // + partHandler.length() != 0
            {
                Net my_net;
                my_net.weights = "src/traced_resnet_model.pt";
                my_net.fileName = partHandler.fileName();
                my_net.labelFile = "src/ImageNetLabels.txt";
                my_net.net_work();

                std::string cls = my_net.cls();

                ostr << "File Name: " << partHandler.fileName() << "\n";
                ostr << "Class: " << cls << "\n";
            }
            else {
                ostr << "File is invalid. Please provide nonempty picture\n";
            }
        }
    }
};
